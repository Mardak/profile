...
Controller.prototype = {
  ...
  getCoverage: function() {
    ...
    Object.keys(coverage).forEach(host => {
      Object.keys(coverage[host]).forEach(nameSpace => {
        Object.keys(coverage[host][nameSpace]).forEach(type => {
          ...
          if (!results[nameSpace][type]) {
            results[nameSpace][type] = {
              totalVisits: 0,
              totalSites: 0,
              coveredVisist: 0,
              coveredSites: 0,
          ...
          results[nameSpace][type].totalVisits += visits;
          results[nameSpace][type].totalSites ++;
          if (interests > 0) {
            results[nameSpace][type].coveredVisist += visits;
            results[nameSpace][type].coveredSites ++;
          }
        });
      });
    });
    ...
    // convert counts to %
     Object.keys(results).forEach(nameSpace => {
      Object.keys(results[nameSpace]).forEach(type => {
        let item = results[nameSpace][type];
        item.coveredVisist = item.coveredVisist * 100 / item.totalVisits;
        item.coveredSites = item.coveredSites * 100 / item.totalSites;
      });
     });
