...
Controller.prototype = {
  ...
  getCoverage: function() {
    let coverage = this._dayBuffer.getCoverage() || {};
    let results = {totalVisits: 0, totalSites: 0, coverage: {}};
    // compute covered visits and distinct covered sites
    Object.keys(coverage).forEach(host => {
      let addToTotal = true;
      Object.keys(coverage[host]).forEach(nameSpace => {
        Object.keys(coverage[host][nameSpace]).forEach(type => {
          let {visits, interests} = coverage[host][nameSpace][type];
          if (!results.coverage[nameSpace]) {
            results.coverage[nameSpace] = {};
          }
          if (!results.coverage[nameSpace][type]) {
            results.coverage[nameSpace][type] = {
              coveredVisits: 0,
              coveredSites: 0,
            };
          }
          ...
          if (interests > 0) {
            results.coverage[nameSpace][type].coveredVisits += visits;
            results.coverage[nameSpace][type].coveredSites ++;
          }
        });
      });
    });
    ...
    // convert counts to %
     Object.keys(results.coverage).forEach(nameSpace => {
      Object.keys(results.coverage[nameSpace]).forEach(type => {
        let item = results.coverage[nameSpace][type];
        item.coveredVisits = Math.round(item.coveredVisits * 100 / results.totalVisits);
        item.coveredSites = Math.round(item.coveredSites * 100 / results.totalSites);
      });
     });
