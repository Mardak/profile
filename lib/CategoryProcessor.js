...
WeightAlgorithm.prototype = {
  ...
  computeWeight: function(placeData) {
     return this.weightFunction(placeData);
  }
}
...
function CategoryProcessor(hostsToCats) {
    ...
    this.weightAlgs.push( new WeightAlgorithm("decay_095",function(placeData,dayDelta) { return placeData.vcount*Math.pow(0.95,dayDelta); }));
    this.weightAlgs.push( new WeightAlgorithm("decay_099",function(placeData,dayDelta) { return placeData.vcount*Math.pow(0.99,dayDelta); }));
    this.weightAlgs.push( new WeightAlgorithm("lin_decay",function(placeData,dayDelta) { return placeData.vcount / (1+dayDelta); }));
...
CategoryProcessor.prototype = {
  ...
  try {
    ...
    if (!this.mySites[domain]) {
      this.mySites[domain] = {vcount: 0 , frecency: 0, power: 0};
    }
    ...
    this.weightAlgs.forEach(function(alg) {
      ...
      Object.keys(matchedCats).forEach(function(cat) {
        if(!this.cats[cat]) this.cats[cat] = {};
        this.cats[cat][name] = (this.cats[cat][name] || 0 ) + contrib;
  ...
  getCategories: function() {
    ...
    let catsCopy = {};
    Object.keys(this.cats).forEach(function(cat) {
      Object.keys(this.cats[cat]).forEach(function(name) {
        catsCopy[cat][name] = Match.floor(this.cats[cat][name] * 1000 / this.largest[name]);
      }.bind(this));
    }.bind(this));
    return catsCopy;
  }
