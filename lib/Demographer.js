 ...
 onReady: function(cb) {
  ...
  finalizeComputations: function() {
    ...
    let resultCats = this.categoryProcessor.getCategories();
      ...
      Object.keys(this.suffixData).forEach(function(suffix) {
        if(this.suffixData[suffix] > 5) {
          // only of there are few URLs in this suffix
          // we need to keep privacy in mind
          let jsonObj = [];
          jsonObj[0] = "SUFFIX";
          jsonObj[1] = suffix;
          jsonObj[2] = this.suffixData[suffix];
          this.results.push(JSON.stringify(jsonObj));
        }
      }.bind(this));
  ...
  extractDomain: function(host) {
    ...
    let re = /^[.]/;
    let host = host.replace(re, "");
    try {
      let tld = Services.eTLD.getBaseDomainFromHost(host);
      if(tld) {
        let suffix = Services.eTLD.getPublicSuffixFromHost(host);
        this.suffixData[suffix] = (this.suffixData[suffix] || 0) + 1;
      }
      return tld;
    } catch(ex) { 
      return null;
  ...
  readHistory: function(offset) {
    ...
    executeQuery("places",query,{offset: offset}, {
        ...
        try {
          ...
          if(!domain) {
            // we failed to determine top level domain 
            // we should then simply exit - it must be the localhost or file
          ...
          // bail if we have no demographics for it
          if (!this.allSites[domain]) {
            return;
          }
