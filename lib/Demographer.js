...
Demographer.prototype = {
 ...
 onReady: function(cb) {
  ...
  finalizeComputations: function() {
    ...
    let outputCats = {};
    let orderedCats = Object.keys(this.cats).sort(function(a, b) {
        return this.cats[b].visist_log - this.cats[a].visist_log;
    }.bind(this));    

    for (let index=0; index < orderedCats.length; index++) {
       let cat = orderedCats[index];
       outputCats[cat] = {visist_log: index, frecency: null, frecency_log: null};
    }

    let rank = 0;
    let self = this;
    Object.keys(this.cats).sort(function(a, b) {
            return self.cats[b].frecency - self.cats[a].frecency;
    }).forEach(function(cat) {
      outputCats[cat].frecency = rank;
      rank++;
    });

    rank = 0;
    Object.keys(this.cats).sort(function(a, b) {
            return self.cats[b].frecency_log - self.cats[a].frecency_log;
    }).forEach(function(cat) {
      outputCats[cat].frecency_log = rank;
      rank++;
    });

    let limit = (orderedCats.length < 50) ? orderedCats.length : 50;
    for (let index=0; index < limit; index ++) {
      jsonObj = [];
      jsonObj[0] = "ODP_CATEGORY";
      jsonObj[1] = orderedCats[index];
      jsonObj[2] = outputCats[orderedCats[index]].visist_log;
      jsonObj[3] = outputCats[orderedCats[index]].frecency;
      jsonObj[4] = outputCats[orderedCats[index]].frecency_log;
      this.results.push( JSON.stringify(jsonObj) );
    }
  },
