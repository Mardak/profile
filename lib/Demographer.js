...
Demographer.prototype = {
 ...
 onReady: function(cb) {
  ...
  finalizeComputations: function() {
    let form_algs = [ this.formHistoryAlg , this.formHistoryNameAlg , this.formHistoryEmailAlg ];
  ...
  analizeFormHistory: function(field,value) {
    ...
    if(field.indexOf("dob") > -1 || field.indexOf("birth") > -1) {
      ...
      if( firstNames["male"][value] ) {
        this.formHistoryNameAlg.addToBuket("male",1);
      }
      if( firstNames["female"][value] ) {
      ...
      if( match ) {
        let name = match[1];
        if( firstNames["male"][name] ) {
          this.formHistoryEmailAlg.addToBuket("male",1);
        }
        if( firstNames["female"][name] ) {
  ...
  readFormHistory: function() {
    ...
    let query = "select fieldname , value from moz_formhistory where fieldname like \"%dob%\" OR " +
