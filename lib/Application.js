...
let StudyApp = {
  ...
  makeSurveyUrl: function _makeSurveyUrl() {
    ...
    let serveyEndPoint = StudyApp.controller.getSurveyEndPoint();
    let orderedInterests = StudyApp.controller.getRankedInterestsForSurvey();

    let url = serveyEndPoint + "?userID=" + StudyApp.controller.getUserID() +
  ...
  start: function _start({loadReason}) {
    ...
    else if(StudyApp.controller.mayPersonalize()) {
