...
let Surveyor = {
  ...
  orderInterestsForSurvey: function(rankings, allInterests, fillNb, localizedInterests) {
    let resultRanking = [];
    let orderedRankings = [];
    ...
    // remap interests nick names to thier localized expressions
    if (localizedInterests) {
      resultRanking.forEach(item => {
        item.interest = encodeURIComponent(localizedInterests[item.interest] || item.interest);
      });
    }
